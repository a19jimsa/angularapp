...
export class Mesh{
    ...
    addSquares(width: number, height: number, sx: number, sy: number, sw: number, sh: number, dx: number, dy: number, dw: number, dh: number){
    ...
    addCube(x: number, y: number, z: number, width: number, height: number, depth: number){
        const w = width / 2;
        const h = height / 2;
        const d = depth / 2;
    ...
    recalculateNormals(quads: number, width: number, height: number){
        ...
        for (let i = 0; i < this.vertices.length; i += 5) {
            positions.push(this.vertices[i], this.vertices[i + 1], this.vertices[i + 2]); // x, y, z
        }
        ...
        for(let i = 0; i < this.normals.length; i+=3){
            const normal = vec3.fromValues(this.normals[i], this.normals[i+1], this.normals[i+2]);
