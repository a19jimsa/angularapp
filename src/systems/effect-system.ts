...
export class EffectSystem {
  update(ecs: Ecs) {
    ...
    const effects = pool.filter(({ entity, components }) => {
      const [effect] = components;
      if (!effect.isAlive) {
        return true;
      } else {
    ...
    effects.forEach(({ entity, components }) => {
      ecs.removeComponent<Effect>(entity, 'Effect');
    });
  }

  runAnimation(effect: Effect) {
    if (effect.sprites.length <= 0) return;
    const totalDuration = ResourceManager.getEffect(
      'effects',
      effect.effectType
    )[ResourceManager.getEffect('effects', effect.effectType).length - 1].time;
    const speed = 1000 / 1;
    ...
    for (const sprite of effect.sprites) {
      for (
        let i = 0;
        i < ResourceManager.getEffect('effects', effect.effectType).length - 1;
        i++
      ) {
        const keyframe = ResourceManager.getEffect(
          'effects',
          effect.effectType
        );
